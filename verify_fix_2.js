...
function parseTest(lines) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (typeIndex !== -1) {
           ...
           // If typeIndex points to a merged string (e.g. Title+Type), clean it
           if (typeIndex === codeIndex + 1) {
               // The parsing logic found type keyword inside parts[codeIndex+1]
               // This implies parts[codeIndex+1] contains BOTH Title and Type
               // We need to extract type and set remaining as title
               for (const kt of knownTypes) {
                   if (parts[typeIndex].toLowerCase().includes(kt.toLowerCase())) {
                       type = kt;
                       title = parts[typeIndex].replace(new RegExp(kt, "i"), "").trim();
                       break;
                   }
               }
           }
